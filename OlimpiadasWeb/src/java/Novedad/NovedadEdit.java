...
public class NovedadEdit extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Parametro reglamento_path = new TParametro().getByCodigo(OptionsCfg.NOVEDAD_PATH);
                ...
                for (FileItem item : items) {
                    ...
                    if (item.isFormField()) {
                        ...
                        if (fieldName.equalsIgnoreCase("fecha")) {
                            fecha = TFecha.formatearFechaVistaBd(fieldValue);
                        }
                        if (fieldName.equalsIgnoreCase("titulo")) {
                            titulo = fieldValue;
                        }
                        if (fieldName.equalsIgnoreCase("subtitulo")) {
                            subtitulo = fieldValue;
                        }
                        if (fieldName.equalsIgnoreCase("youtube")) {
                            youtube = fieldValue;
                        }
                        if (fieldName.equalsIgnoreCase("detalle")) {
                            detalle = fieldValue;
                        }
                    } else {
                        ...
                        if (fieldName.equalsIgnoreCase("imagen")) {
                            archivo1 = item;
                            imagen = FilenameUtils.getName(item.getName().replaceAll(" ", "_"));
                        }
                    }
                };
...
//                    TNotificacion tn = new TNotificacion();
                 //   tn.notificar_novedad(id);
                }
            } else {
                todoOk = tnovedad.actualizar(novedad);
                id = novedad.getId();
            }
            if (!todoOk) {
