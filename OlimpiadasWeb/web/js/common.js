...
function siguienteHora(fecha, hora) {
    ...
    if (hora === "")
        hora = moment().subtract(1, 'hour').format('HH:mm:ss');  // Tomamos una hora antes
...
function movePage(side) {
    ...
    data.pagNro = page;
    if (page == 0) {
        $('.data-left').css('visibility', 'hidden')
    }
    if (page > 0) {
        $('.data-left').css('visibility', 'visible')
    }
    loadData(data);
}
...
function filtrar(e) {
    ...
    if ($hora_hasta.val())
        data.hora_hasta = $hora_hasta.val();
    else {
...
function rubroChange(url, data, $id_subrubro) {
    ...
    if (window.loadDataActivo)
        loadDataActivo(data);
    if (window.loadDataKit)
        loadDataKit(data);
}
...
function setLoaderDiv($div) {
    $div.html("<center><img src='images/ajax-loader.gif'/></center>");
}
